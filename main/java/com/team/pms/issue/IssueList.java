...
//		List<IssueDTO> list = dao.issuelist(employeeSeq);
		List<IssueDTO> list = dao.list(map);
		...
		if (n == 1) {
			...
			sb.append(String.format("<a href=\"/pms/issue/issuelist.do?page=%d\">[이전 %d페이지]</a>", n - 1, blockSize));
		}
		...
		while (!(loop > blockSize || n > totalPage)) {
			...
			if (n == nowPage) {
				...
				sb.append(String.format(" <a href=\"/pms/issue/issuelist.do?page=%d\">%d</a> ", n, n));
			}
		...
		if (n > totalPage) {
			...
			sb.append(String.format("<a href=\"/pms/issue/issuelist.do?page=%d\">[다음 %d페이지]</a>", n, blockSize));
		}
...
//		req.setAttribute("list", list);
		...
		RequestDispatcher dispatcher = req.getRequestDispatcher("/WEB-INF/views/issue/issuelist.jsp");
